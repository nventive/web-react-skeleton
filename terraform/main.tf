...
resource "azurerm_storage_account" "sa_webapp" {
...
resource "azurerm_cdn_endpoint" "cdne" {
  ...
  origin_host_header            = replace(azurerm_storage_account.sa_webapp.primary_web_host, "https://", "")

  origin {
    name       = var.project_short_name
    host_name  = replace(azurerm_storage_account.sa_webapp.primary_web_host, "https://", "")
    https_port = "443"
  }
